...
export const submitStockTransferAndProductBatch = async (req, res) => {
  try {
    ...
    const productBatchResult = await TransferProductBatchService.createStockTransferProductBatchService(productBatchEntries);

      const result = await Service.updateStockTransferSubmitted(STN_NO, submittedBy);
...
export const updateOrAddProductBatchSumController = async (req, res) => {
  try {
    ...
    const result = await ProductBatchSumService.updateOrAddProductBatchSum( requestBranch, products);
