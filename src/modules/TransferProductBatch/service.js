...
export const createStockTransferProductBatchService = async (productBatchEntries) => {
  try {
    ...
    for (const entry of productBatchEntries) {
      try {
        ...
        await ProductBatchSumService.updateProductBatchSumQty(entry.productId, entry.batchNo, entry.supplyingBranch, entry.transferQty);
    ...
    console.error("Error creating Stock Transfer Product Batch:", error.message);
    throw new Error("Error creating Stock Transfer Product Batch: " + error.message);
  }
};
