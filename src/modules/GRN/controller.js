...
export const createGRNAndProduct = async (req, res) => {
  try {
    ...
    if (result.success) {
      const productIds = [
        ...new Set(productGRNs.map((product) => product.productId)),
      ];
      //await updateProductQty(productIds);
      res
        .status(201)
        .json({
          message: "GRN and productGRN entries created successfully",
          newProductGRNs: result.newProductGRNs,
        });
    } else {
      res
        .status(400)
        .json({ message: "Validation error creating productGRN entries" });
    }
  } catch (error) {
    ...
    res
      .status(500)
      .json({ message: "Failed to create GRN and productGRN entries" });
  }
};
