...
export const updateStockTransferSubmitted = async (STN_NO, submittedBy) => {
  try {
    const stockTransferEntry = await stockTransfer.findOne({
      where: { STN_NO },
    });
...
export const cancelStockTransfer = async (STN_NO, submittedBy) => {
  const [err, stockTransferRecord] = await to(
    stockTransfer.findOne({
      where: { STN_NO },
    })
  );
  if (err) TE(err);
      ...
      stockTransfers.map(async (transfer) => {
        ...
        const productsWithDetails = await Promise.all(
          productData.map(async (product) => {
            ...
            if (!productInfo) {
              throw new Error(
                `Product not found for productId: ${product.productId}`
              );
            }
            return {
              ...product.dataValues,
